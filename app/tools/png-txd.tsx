...
import { parsePng } from '@/src/converters/pngutil';
import { encodeTxd } from '@/src/converters/txdpng';
import { pickFile, readFileBinary, saveAndShare } from '@/src/utils/files';
import * as Haptics from 'expo-haptics';

export default function PngTxdScreen() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const texName = selectedFile.name.replace(/\.png$/i, '');
      const txdData = encodeTxd(texName, img.width, img.height, img.rgba);
      const outName = texName + '.txd';
      await saveAndShare(outName, txdData, 'application/octet-stream');
      await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      setResult({ success: true, message: `${outName} (${img.width}×${img.height}) создан!` });
    } catch (e: any) {
  ...
  return (
    <View style={styles.container}>
      <ProcessingOverlay visible={processing} message="Создание TXD..." />
      <ScrollView
      ...
      >
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            Создаёт TXD файл из PNG изображения. Текстура сохраняется без сжатия (ARGB8888). Имя текстуры берётся из имени файла.
          </Text>
        </View>
        ...
        <TouchableOpacity style={styles.filePicker} onPress={handlePickFile} activeOpacity={0.7}>
          ...
          <Text style={[styles.filePickerText, selectedFile && styles.filePickerTextActive]}>
            {selectedFile ? selectedFile.name : 'Нажмите для выбора .png файла'}
          </Text>
        ...
        >
          ...
          <Text style={styles.buttonText}>Создать TXD из PNG</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
